...
public class CustomeSwipeRefreshLayout extends ViewGroup {
    ...
    private static final int DEFAULT_CIRCLE_TARGET = 64;// 64;
    private static final int[] LAYOUT_ATTRS = new int[]{android.R.attr.enabled};
    private final DecelerateInterpolator mDecelerateInterpolator;
    public boolean mEnablePullLoad = true; // 是否支持上拉刷新
    protected int mFrom;
    protected int mOriginalOffsetTop;
    // private String push_to_refresh_push_label = "上拉加载更多…";
    // private String push_to_refresh_release_label = "松开加载…";
    // private String pull_to_refresh_pull_label = "下拉刷新…";
    // private String pull_to_refresh_release_label = "松开刷新…";
    // private String pull_to_refresh_refreshing_label = "加载中…";
    ...
    private TextProgressView defaultProgressView = null;
    private TextProgressView loadMoreProgressView = null;
    ...
    public CustomeSwipeRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        defaultProgressView = new TextProgressView(getContext());
        // 上拉刷新
        loadMoreProgressView = new TextProgressView(getContext());
        loadMoreProgressView.pull_to_refresh_pull_label = "上拉加载更多…";
        loadMoreProgressView.pull_to_refresh_release_label = "松开加载…";
        // defaultProgressView = new CircleProgressView(getContext());
        createHeaderViewContainer();
        createFooterViewContainer();
        ViewCompat.setChildrenDrawingOrderEnabled(this, true);
        mSpinnerFinalOffset = (DEFAULT_CIRCLE_TARGET / 2) * metrics.density;
    ...
    private void createHeaderViewContainer() {
        RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(
                (int) (mHeaderViewWidth * 0.8), (int) (mHeaderViewHeight * 0.8));
    ...
    private void createFooterViewContainer() {
        ...
        mFooterViewContainer.addView(loadMoreProgressView, layoutParams);
    ...
    public void autoRefresh() {
        ...
        if (!mRefreshing) {
            ...
            setRefreshing(true, false /* notify */);
            defaultProgressView.endLoding();
            // defaultProgressView.setOnDraw(false);
        }
    }
    ...
    public void setRefreshing(boolean refreshing) {
        ...
        if (mLoadMore && !refreshing)// 上拉加载更多、停止操作
            setLoadMore(false);
    }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mHeadViewContainer.measure(MeasureSpec.makeMeasureSpec(mHeaderViewWidth, MeasureSpec.EXACTLY),
                MeasureSpec.makeMeasureSpec(1 * mHeaderViewHeight, MeasureSpec.EXACTLY));
...
//        //Logger.v("YESWAYBLOG","last position:"+lastCompletelyVisibleItemPosition);
//        if (lastCompletelyVisibleItemPosition == count - 1) {
//          //Logger.v("YESWAYBLOG","last position succ");
//          return true;
//        }
...
//      return false;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mActivePointerId == INVALID_POINTER) {
                    // Log.e(LOG_TAG, "Got ACTION_MOVE event but don't have an
                    // active pointer id.");
                    return false;
                }
                ...
                float yDiff = 0;
                if (mEnablePullLoad && mOnPushLoadMoreListener != null && isChildScrollToBottom()
                        && mInitialMotionY - y > 0) {
                    yDiff = mInitialMotionY - y;// 计算上拉距离
                    if (yDiff > mTouchSlop && !mIsBeingDragged) {// 判断是否下拉的距离足够
                        mIsBeingDragged = true;// 正在上拉
                    }
                } else if (mEnablePullRefresh && mListener != null && isChildScrollToTop()
                        && mInitialMotionY - y < 0) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mEnablePullLoad && mOnPushLoadMoreListener != null && isChildScrollToBottom()
                && mInitialMotionY - ev.getY() > 0) {// 上拉加载更多
            return handlerPushTouchEvent(ev, action);
        } else if (mEnablePullRefresh && mListener != null && isChildScrollToTop()
                && mInitialMotionY - ev.getY() < 0) {// 下拉刷新
    ...
    private boolean handlerPullTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (pointerIndex < 0) {
                    // Log.e(LOG_TAG, "Got ACTION_MOVE event but have an invalid
                    // active pointer id.");
                    return false;
                }
                ...
                if (mIsBeingDragged && mListener != null) {
                    ...
                    if (overscrollTop < mTotalDragDistance) {
                        ...
                        if (mListener != null) {
                            defaultProgressView.refreshPull();
                        }
                    } else {
                        if (mListener != null) {
                            defaultProgressView.release();
                        }
                    }
            ...
            case MotionEvent.ACTION_CANCEL: {
                if (mActivePointerId == INVALID_POINTER) {
                    if (action == MotionEvent.ACTION_UP) {
                        // Log.e(LOG_TAG, "Got ACTION_UP event but don't have an
                        // active pointer id.");
                    }
                    return false;
                }
                ...
                final float overscrollTop = (y - mInitialMotionY) * DRAG_RATE;
                mIsBeingDragged = false;
                if (overscrollTop > mTotalDragDistance && mListener != null) {
    ...
    private boolean handlerPushTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged && mOnPushLoadMoreListener != null) {
                    ...
                    if (loadMoreProgressView != null) {
                        if (pushDistance >= mFooterViewHeight)
                            loadMoreProgressView.release();
                        else
                            loadMoreProgressView.refreshPull();
                    }
                }
                break;
            }
            ...
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL: {
                if (mOnPushLoadMoreListener == null) // 没有上拉加载监听,则不进行计算
                    break;
                if (mActivePointerId == INVALID_POINTER) {
                    if (action == MotionEvent.ACTION_UP) {
                        // Log.e(LOG_TAG, "Got ACTION_UP event but don't have an
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
                    ...
                    if (pushDistance == mFooterViewHeight && mOnPushLoadMoreListener != null) {
                        ...
                        if (loadMoreProgressView != null) loadMoreProgressView.loding();
                    }
                } else {
    ...
    public void setLoadMore(boolean loadMore) {
        if (!loadMore && mLoadMore) {// 停止加载
            if (loadMoreProgressView != null) loadMoreProgressView.endLoding();
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
                mLoadMore = false;
                pushDistance = 0;
                updateFooterViewPosition();
     ...
     * 文字的下拉刷新View
     */
    static public class TextProgressView extends LinearLayout {
        String pull_to_refresh_pull_label = "下拉刷新…";
        String pull_to_refresh_release_label = "松开刷新…";
        String pull_to_refresh_refreshing_label = "加载中…";
        ...
        private void init(Context context) {
            ...
            imageView.setImageResource(R.mipmap.default_ptr_rotate);
            addView(imageView, params);

            textView = new TextView(context);
            textView.setTextColor(0xff909090);
            textView.setText("加载中...");
            addView(textView);

            animationRotate = new RotateAnimation(0.0f, 360.0f, Animation.RELATIVE_TO_SELF, 0.5f,
        ...
        public void loding() {
            textView.setText(pull_to_refresh_refreshing_label);
            // 旋转进度菊花
            imageView.setAnimation(animationRotate);
        }

        public void refreshPull() {
            textView.setText(pull_to_refresh_pull_label);
        }

        public void release() {
            textView.setText(pull_to_refresh_release_label);
        }
    }
